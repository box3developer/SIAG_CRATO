    ...
    public class SetorTrabalhoRepository : BaseRepository<SetorTrabalho, int>, ISetorTrabalhoRepository
    {
        ...
        private IQueryable<SetorTrabalho> FiltroPesquisa(IQueryable<SetorTrabalho> query, string pesquisa)
        {
            if (!string.IsNullOrWhiteSpace(pesquisa))
            {
                ...
                query = query.Where(x => EF.Functions.Like(x.SetorTrabalhoId.ToString(), pesquisa) ||
        ...
        public async Task<DadosPaginadosDTO<SetorTrabalho>> GetListAsync(FiltroPaginacaoDTO dto)
        {
            ...
            var lista = await query.OrderByDescending(x => x.SetorTrabalhoId)
        ...
        public async Task<List<SelectDTO<int>>> GetSelectAsync(FiltroPaginacaoDTO dto)
        {
            ...
            query = query.OrderBy(x => x.SetorTrabalhoId)
                         .Take(30);

            var dados = await query
                .Select(x => new SelectDTO<int>
                {
                    Id = x.SetorTrabalhoId,
