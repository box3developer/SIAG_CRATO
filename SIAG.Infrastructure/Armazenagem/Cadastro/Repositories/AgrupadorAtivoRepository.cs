    ...
    public class AgrupadorAtivoRepository : BaseRepository<AgrupadorAtivo, string>, IAgrupadorAtivoRepository
    {
        ...
        private IQueryable<AgrupadorAtivo> FiltroPesquisa(IQueryable<AgrupadorAtivo> query, string pesquisa)
        {
            if (!string.IsNullOrWhiteSpace(pesquisa))
            {
                ...
                query = query.Where(x => EF.Functions.Like(x.AgrupadorId.ToString(), pesquisa) ||
        ...
        public async Task<DadosPaginadosDTO<AgrupadorAtivo>> GetListAsync(FiltroPaginacaoDTO dto)
        {
            ...
            var lista = await query.OrderByDescending(x => x.AgrupadorId)
        ...
        public async Task<List<SelectDTO<string>>> GetSelectAsync(FiltroPaginacaoDTO dto)
        {
            ...
            query = query.OrderBy(x => x.AgrupadorId)
                         .Take(30);

            var dados = await query
                .Select(x => new SelectDTO<string>
                {
                    Id = x.AgrupadorId.ToString(),
