...
namespace SIAG.Application.Armazenagem.Cadastro.Services
{
    public class ProgramaService : BaseService<IProgramaRepository, Programa, ProgramaDTO, int>
    {
        ...
        public async Task<DadosPaginadosDTO<ProgramaDTO>> GetListAsync(FiltroPaginacaoDTO dto)
        {
            var lista = await _repository.GetListAsync(dto);

            var listaFormatada = lista.Dados.Select(x => _mappingService.Map<Programa, ProgramaDTO>(x)).ToList();

            return new DadosPaginadosDTO<ProgramaDTO>
            {
