...
{
    public class PalletService : BaseService<IPalletRepository, Pallet, PalletDTO, int>
    {
        ...
        public async Task<DadosPaginadosDTO<PalletDTO>> GetListAsync(FiltroPaginacaoDTO dto)
        {
            var lista = await _repository.GetListAsync(dto);

            var listaFormatada = lista.Dados.Select(x => _mappingService.Map<Pallet, PalletDTO>(x)).ToList();

            return new DadosPaginadosDTO<PalletDTO>
            {
