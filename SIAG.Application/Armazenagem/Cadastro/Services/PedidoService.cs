...
{
    public class PedidoService : BaseService<IPedidoRepository, Pedido, PedidoDTO, int>
    {
        ...
        public async Task<DadosPaginadosDTO<PedidoDTO>> GetListAsync(FiltroPaginacaoDTO dto)
        {
            var lista = await _repository.GetListAsync(dto);

            var listaFormatada = lista.Dados.Select(x => _mappingService.Map<Pedido, PedidoDTO>(x)).ToList();

            return new DadosPaginadosDTO<PedidoDTO>
            {
